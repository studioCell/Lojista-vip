...
const Feed: React.FC = () => {
  ...
  const handleStoryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
        ...
        // This is crucial when using Firestore as "Storage" via Base64
  ...
  useEffect(() => {
    ...
    if (currentStory) {
        ...
        // If video, we use onEnded event on the video tag
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 800 * 1024) {
          alert("Imagem muito grande. Tente usar uma menor.");
          return;
      }
      const url = await fileToBase64(file);
      setPreviewUrl(url);
  ...
  const handlePublish = (e: React.FormEvent) => {
    ...
    addOffer({
      ...
      timestamp: '' // generated in context
    });
